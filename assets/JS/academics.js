cObj("registersub").onclick = function () {
    ...
    if (err==0) {
        ...
        if (select>0) {
            ...
            var datastring = "?addsubject=true&subjectname="+valObj("subname")+"&subjectmax="+valObj("submarks")+"&claslist="+classed+"&subids="+valObj("sundids")+"&grades_lists="+set_my_grades_list;
...
cObj("updatesubs").onclick = function () {
    ...
    if (err==0) {
        ...
        var datapass = "?updatesubjects=true&subname="+valObj("subnam")+"&subttid="+valObj("subidss")+"&submaxmarks="+valObj("submarksd")+"&classtaught="+classelected+"&subjeid="+cObj("subids").innerText+"&subject_grade="+subject_grade;
    ...
    if (cObj("examform2").classList.contains("hide")) {
        ...
        if (err == 0) {
            ...
            if (checkedsub > 0) {
                ...
                }else if (cObj("curriculum").value == "844" || cObj("curriculum").value == "IGCSE") {
...
function viewClassListener() {
    ...
    if (cObj("imager3_e"+idds).classList.contains("hide")) {
        ...
        if (err == 0) {
            var datapassing = "?save_student_marks="+cObj("input"+idds).value+"&examidds="+cObj("exam_list").value+"&subjectidds="+cObj("sub_jectlists").value+"&subject_grade="+data+"&grade_method="+cObj("grade_mode").value+"&student_ids="+idds+"&class_name="+cObj("class_siter").innerText;
...
function changeMarksListener() {
    ...
    cObj("gradeMethod").innerText = cObj("my_curriculum").innerText;
    ...
    if (grade.length > 2 && (grade == "E.E" || grade == "M.E" || grade == "A.E" || grade == "B.E")) {
        ...
        if (grade!="Absent") {
            cObj("idd"+cObj("subject_marks"+recordid).innerText.trim()).selected = true;
        }else{
...
cObj("save_marks_change").onclick = function () {
    ...
    if (cObj("cbcmode1").classList.contains("hide")) {
        ...
        datapass+="&valued="+cObj("cbc_mode1").value;
...
cObj("delete-subject").onclick = function () {
    cObj("mssg_name").innerText = "Are you sure you want to delete <b>"+cObj("subnam").value+"</b> ?";
            ...
            for (let index2=0; index2 < classes.length; index2++) { 
                ...
                for (let d = 0; d < classes.length; d++) {
                    ...
                    for (let e = 0; e < elc.lessons.length; e++) {
                        ...
                        var new_lesson = eld.trim().split("=").length > 1 ? eld.trim().split("=")[1].trim():"";
                ...
                for (let index3=0; index3 < lessons.length; index3++) { 
                    ...
                    for (let index4=0; index4 < all_lessons.length; index4++) { 
                        ...
                        var detailed_lesson = class_lessons[index3].trim().split("=");
    ...
    if (timetable_blocks.length > 0) {
        ...
        for (let index=0; index < timetable_blocks.length; index++) { 
            ...
            for (let index2=0; index2 < classes.length; index2++) {
                ...
                // loop through lessons for a particular class and find if they have ever occured
                for (let index3=0; index3 < lessons.length; index3++) { 
                    var single_lesson = lessons[index3].trim();
                    let present = checkPresent(posible_lessons[class_names],single_lesson);
                    if (!present) {
                        posible_lessons[class_names].push(single_lesson);
                    }
