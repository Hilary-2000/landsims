...
function displayRecord2(start, finish, arrays) {
    ...
    //set the page number
    cObj("pagenumNav2").innerText = pagecounttrans;
...
cObj("tonextNav2").onclick = function () {
    if (pagecounttrans < pagecountTransaction) { // if the current page is less than the total number of pages add a page to go to the next page
        ...
        cObj("transDataReciever2").innerHTML = displayRecord2(startpage, endpage, rowsColStudents2);
        set_routes();
    } else {
...
cObj("de_register_stud_transport").onclick = function () {
    ...
    var datapass = "deregister_stud="+valObj("stud_detail_trans_id");
    sendDataPost("POST","/sims/ajax/administration/admissions.php",datapass,cObj("err_handler_transport2"),cObj("update_std_spinner"));
    setTimeout(() => {
        var timeout = 0;
        var idfs = setInterval(() => {
