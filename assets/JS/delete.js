...
function displayRecord2(start, finish, arrays) {
    ...
    tableData += "</table>"; cObj("startNo2").innerText = (start + 1); cObj("finishNo2").innerText = fins; cObj("pagenumNav2").innerText = pagecounttrans; return tableData;
}
cObj("tonextNav2").onclick = function () {
    if (pagecounttrans < pagecountTransaction) { startpage += 10; pagecounttrans++; var endpage = startpage + 11; cObj("transDataReciever2").innerHTML = displayRecord2(startpage, endpage, rowsColStudents2); set_routes(); } else { pagecounttrans = pagecountTransaction; }
    setAssignLis();
}
cObj("toprevNac2").onclick = function () {
    if (pagecounttrans > 1) { pagecounttrans--; startpage -= 10; var endpage = (startpage + 10) - 1; cObj("transDataReciever2").innerHTML = displayRecord2(startpage, endpage, rowsColStudents2); set_routes(); }
    setAssignLis();
}
cObj("tofirstNav2").onclick = function () {
    if (pagecountTransaction > 0) { pagecounttrans = 1; startpage = 0; var endpage = startpage + 10; cObj("transDataReciever2").innerHTML = displayRecord2(startpage, endpage, rowsColStudents2); set_routes(); }
    setAssignLis();
}
cObj("tolastNav2").onclick = function () {
    if (pagecountTransaction > 0) { pagecounttrans = pagecountTransaction; startpage = ((pagecounttrans * 10) - 10) + 1; var endpage = startpage + 10; cObj("transDataReciever2").innerHTML = displayRecord2(startpage, endpage, rowsColStudents2); set_routes(); }
    setAssignLis();
}
...
var rowsColStudents1 = []; var pagecountTransaction1 = 0; var pagecounttrans1 = 1; var startpage1 = 1; function displayRecord1(start, finish, arrays) {
    start--; if (start < 0) { start = 0; }
