<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        ...
        if (isset($_GET['notices'])) {
            ...
            $select = "SELECT `notification_id`,`notification_name` FROM `tblnotification` WHERE `notification_status` = 0 AND (`notification_reciever_id` = ? OR `notification_reciever_id` = 'all') AND (`notification_reciever_auth` = ? OR `notification_reciever_auth` = 'all')";
            $stmt = $conn2->prepare($select);
            $stmt->bind_param("ss",$_SESSION['userids'],$_SESSION['authority']);
            $stmt->execute();
            $result = $stmt->get_result();
