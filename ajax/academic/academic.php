<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        ...
        elseif (isset($_GET['get_user_roles'])) {
            ...
            if ($present != true) {
                ...
                $subactive = 1;
                $insert = "insert INTO `table_subject` (`subject_name`,`max_marks`,`classes_taught`,`sub_activated`,`timetable_id`,`grading`) VALUES (?,?,?,?,?,?)";
                $stmt = $conn2->prepare($insert);
                $stmt->bind_param("ssssss",$subjectname,$subjmax,$classes,$subactive,$sundids,$grades_lists);
                if($stmt->execute()){
            ...
            if ($find == "byname") {
                $name = "%".$_GET['subjename']."%";
                $select = 'SELECT `subject_id`,`subject_name`,`max_marks`,`timetable_id`,`classes_taught`,`teachers_id`,`sub_activated` FROM `table_subject` WHERE `subject_name` like ?';
                $stmt = $conn2->prepare($select);
                ...
                $class = "%".$_GET['class']."%";
                $select = 'SELECT `subject_id`,`subject_name`,`max_marks`,`timetable_id`,`classes_taught`,`teachers_id`,`sub_activated` FROM `table_subject` WHERE `classes_taught` LIKE ?';
                $stmt = $conn2->prepare($select);
            ...
            if ($result) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $tableinformation.="<tr><td>".$xs."</td><td>".$row['subject_name']."</td><td>".$ttsd."</td><td>".$row['max_marks']."</td><td>".$row['classes_taught']."</td><td><p style='font-size:12px;;margin:0 auto;' class ='viewsubj link' value='".$row['subject_id']."' id='class".$row['subject_id']."'><i class='fas fa-pen'></i> Edit</p></td></tr>";
                }
            ...
            if ($result) {
                if ($row = $result->fetch_assoc()) {
                    $data.=$row['subject_id']."&".$row['subject_name']."&".$row['max_marks']."&".$row['classes_taught']."&".$row['sub_activated']."&".$row['timetable_id']."&".$row['grading'];
                }
            }
            if (strlen($data)>0) {
                echo $data;
        ...
        }elseif (isset($_GET['delete_roles'])) {
            include("../../connections/conn1.php");
            $delete_roles = $_GET['delete_roles'];
            $schcodes = $_SESSION['schcode'];
        ...
        elseif (isset($_GET['updatesubjects'])) {
            ...
            $update = "UPDATE `table_subject` SET `subject_name` = ?,`timetable_id` = ?,`max_marks` = ?, `classes_taught` = ?, `grading` = ? WHERE `subject_id` =?";
            $stmt= $conn2->prepare($update);
            $stmt->bind_param("ssssss",$subname,$ttid,$maxmark,$classtaught,$subject_grade,$subid);
            if($stmt->execute()){
            ...
            $select = "SELECT `subject_id`,`subject_name`,`max_marks`,`timetable_id`,`classes_taught`,`teachers_id`,`sub_activated` FROM `table_subject`";
            $stmt = $conn2->prepare($select);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result) {
            ...
            if ($results) {
                ...
                while ($row = $results->fetch_assoc()) {
                    ...
                    $tableinformation.="<tr><td>".$xs."</td><td>".$row['fullname']."</td><td>".$subjectlist."</td><td>"."<span class='setSubclass link' id='sub".$row['user_id']."' ><i class ='fa fa-pen'></i> Edit </span>"."</td></tr>";
                }
            ...
            if ($_GET['getExamination'] == "allactive") {
                ...
                $datetoday = date("Y-m-d",strtotime("3 hour"));
                ...
                if ($res) {
                    ...
                    $stmt->execute();
                    echo "<p style='text-align:center;'>These are exams registered this term</p>";
                }
            }
            if ($stmt) {
                $results = $stmt->get_result();
                if ($results) {
                    $datatoecho = "<h5 class='text-center mt-2'><b>Examination Table</b></h5><div class='table_holders'><table class='table'>
                                    <tr>
                                        <th>#</th>
                                        <th>Exam name</th>
                                        <th>Status</th>
                    ...
                    while ($row = $results->fetch_assoc()) {
                        ...
                        $todaysdate = date_create(date("Y-m-d",strtotime("3 hour")));
                        ...
                        $datatoecho.="<tr>
                                        ...
                                        <td><span type='button' style='font-size:12px;' class='viewExams link' id='examview".$row['exams_id']."' ><i class ='fa fa-eye'></i> View</span>  <span type='button' style='font-size:12px;' id='prints_exams".$row['exams_id']."' class='link prints_exams'><i class ='fa fa-print'></i> Print</span></td>
                                    </tr>";
                    }
            ...
            if ($result) {
                if ($row = $result->fetch_assoc()) {
                    ...
                    if (strlen($tr_subjects)>0) {
                        $tr_subjects = substr($tr_subjects,0,strlen($tr_subjects)-1);
            ...
            if ($result) {
                ...
                if (strlen($subject_list) > 0) {
                    ...
                    for ($spl=0; $spl < count($split_class_tr); $spl++) { 
                        $in41 = substr($split_class_tr[$spl], 1, strlen($split_class_tr[$spl]) - 2);
                        $in42 = explode(":",$in41);
                        $tr_id = $in42[0];
                        $sub_cl = $in42[1];
                        if ($tr_id == $user_id || $present == 1) {
                            $string_in.=$sub_cl.",";
                        }
                    }
                    // echo $string_in;
                    $string_in = substr($string_in,0,strlen($string_in)-1);
                    $class_list = explode(",",$string_in);
                    ...
                    $class_to_display = "<label class='form-control-label' for='cls_lists' >Select the class to record marks: <br></label><select class='form-control' name='cls_lists' id='cls_lists'>
                                        <option value='' hidden>Select an option..</option>";
                    for ($df=0; $df < count($class_list); $df++) { 
                        $datas = "Class ".$class_list[$df];
                        if (strlen($class_list[$df])>1) {
                            $datas = $class_list[$df];
                        }
                        $in_list = checkPresnt($exam_classes,$class_list[$df]);
                        if ($in_list == 1) {
                            $class_to_display.="<option value='".$class_list[$df]."'>".$datas."</option>";
                            $counter++;
                        }
                    }
                    $class_to_display.="</select>";
                    if ($counter > 0 && strlen($string_in) > 0) {
                        echo $class_to_display;
                    }else {
                        echo "<p style='color:green;'>You dont teach any class of the selected subject or the classes are not set in the exam manager<br>Please select another subject!</p>";
                    }
                }
            }
            ...
            $select = "SELECT `adm_no`, `first_name`,`second_name`,`stud_class` FROM `student_data` WHERE `stud_class` = ? AND `deleted` = 0 AND `activated` = 1";
            $stmt = $conn2->prepare($select);
            ...
            if ($result) {
                ...
                $data_to_display = $grade_table."<h6 style='text-align:center;'>Exam : <span>".getExamName($exam_ids,$conn2)."</span> <span class='hide' id = 'exams_grades' >".$subject_grades."</span></h6>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($present == 0) {
                        ...
                        $xd++;
                        $selection = "";
                        if ($curriculum == "844" || $curriculum == "IGCSE") {
                            ...
                            $selection = "<select class='selected_grade form-control mb-2' style='min-width:100px' name='input".$adm_no."' id='input".$adm_no."'>
            ...
            $insert = "INSERT INTO `exam_record_tbl` (`exam_id`,`student_id`,`subject_id`,`exam_marks`,`exam_grade`,`filled_by`,`class name`) VALUES (?,?,?,?,?,?,?)";
            $stmt = $conn2->prepare($insert);
            $userids = $_SESSION['userids'];
            $stmt->bind_param("sssssss",$examination_id,$student_id,$subject_id,$subject_marks,$subject_grade,$userids,$class_name);
            $stmt->execute();
            ...
            $select = "SELECT `result_id`,`exam_id`,`student_id`,`subject_id`,`exam_marks`,`exam_grade`,`filled_by` FROM `exam_record_tbl` WHERE `subject_id` = ? AND `exam_id` = ? AND `class name` = ? ORDER BY `exam_marks` DESC";
            $stmt = $conn2->prepare($select);
            ...
            if ($result) {
                ...
                $today = date_create(date("Y-m-d",strtotime("3 hour")));
        ...
        elseif (isset($_GET['get_perfomance_for_class'])) {
                                ...
                                if ($result) {
                                    ...
                                    if (count($student_ids) > 0) {
                                        ...
                                        if (count($row_data) > 0) {
                                            ...
                                            $data_to_display="<div id='exam-results-in' style='text-align:center;'><h6 class='my-0'>".getExamName($exam_id,$conn2)."</h6>
                                                            <h6>".className($class_sat)." Results</h6>
                                                            </div>";
                                            $data_to_display.="<div class='tableHolder'><table><tr><th>Pos.</th><th>Student Name</th>";
                                            ...
                                            foreach ($grand_total as $keys => $values) {
                                                ...
                                                for ($inter=0; $inter < count($column_data); $inter++) { 
                                                    ...
                                                    if ($inter == 0) {
                                                        ...
                                                        $data_to_display.="<td>".getStudentName($datacol,$conn2)."<small style='color:brown;'>(".$datacol.")</small></td>";
                                                    }else {
                                            ...
                                            if ($xsd > 6) {
                                                ...
                                                foreach ($grand_total as $keys => $values) {
                                                    ...
                                                    $rowindexs = $keys;
                                                    $student_name = getStudentName($row_data[$rowindexs][0],$conn2);
                                                    if ($grand_total[$keys]/count($last_subjects) < 5) {
                                                        ...
                                                        $data_to_display.="<tr><td>".$xm." 🏆 </td> <td><span style='color:green;font-size:11px;'>".$student_name."</span> <small>(".$row_data[$rowindexs][0].")</small></td> <td>".$my_totals."</td></tr>";
                                                    }else {
                                                        $data_to_display.="<tr><td>".$xm." 🏆 </td> <td><span style='color:green;font-size:11px;'>".$student_name."</span> <small>(".$row_data[$rowindexs][0].")</small></td> <td>".$grand_total[$keys]."</td></tr>";
                                                    }
                                                }
                                                ...
                                                foreach ($grand_total as $keys => $values) {
                                                    ...
                                                    if (count($grand_total)-$xmd == $xmd2) {
                                                        $student_name = getStudentName($row_data[$keys][0],$conn2);
                                                        if ($grand_total[$keys]/count($last_subjects) < 5) {
                                                            $final_total = $grand_total[$keys]/count($last_subjects);
                                                            $my_totals = "-";
                                                            if ($final_total >= 3.5 && $final_total < 4) {
                                                            ...
                                                            $data_to_display.="<tr><td>".$xmd." 🚩</td><td><span style='color:brown;font-size:11px;'>".$student_name."</span> <small>(".$row_data[$keys][0].")</small></td><td>".$my_totals."</td></tr>";
                                                        }else {
                                                            $data_to_display.="<tr><td>".$xmd." 🚩</td><td><span style='color:brown;font-size:11px;'>".$student_name."</span> <small>(".$row_data[$keys][0].")</small></td><td>".$grand_total[$keys]."</td></tr>";                                                            
                                                        }
                                                        $xmd2--;
                                                    }
                                                }
                                            ...
                                            $data_to_display.="<p><br><strong>Hint</strong><br>\t- Hover your mouse over the subject name to get its full name</p>
                                                                <p><strong>Key to Grading system</strong><br>1. <strong>E.E</strong> - Exceeding Expectation<br>2. <strong>M.E</strong> - Meeting Expectation<br>3. <strong>A.E</strong> - Approaching Expectation<br>4. <strong>B.E</strong> - Below Expectation<br>5. <strong>A</strong> - Absent <br></p>
                                                                </div>";
                                            if ($xsd > 0) {
                                                if ($cbc_1 > 0 || $ei44 > 0) {
                                                    if ($cbc_1 > 0 && $ei44 > 0) {
            ...
            $select = "SELECT `subject_id`,`subject_name`,`timetable_id` FROM `table_subject` WHERE `classes_taught` LIKE ?";
            $stmt = $conn2->prepare($select);
            ...
            for ($index=0; $index < count($class_arr); $index++) { 
                ...
                if ($result) {
                    while ($row = $result->fetch_assoc()) {
                        $string_data = $row['subject_id'].":".$row['subject_name'].":".$row['timetable_id'];
            ...
            $data_to_display = "<div class = 'checkboxholder' style='margin:10px 0;padding:0px 0px;'><label style='margin-right:5px;cursor:pointer;font-size:12px;'>No Subjects available</label></div>";
            ...
            $select = "SELECT `subject_name`,`teachers_id`,`classes_taught` FROM `table_subject` WHERE `subject_id` = ?";
            $stmt = $conn2->prepare($select);
            ...
            for ($index=0; $index < count($exp_sub); $index++) { 
                ...
                if ($result) {
                    if ($row = $result->fetch_assoc()) {
                        ...
                        $data_to_display.=($index+1).". <strong>".$row['subject_name']."</strong>: <br>";
                        $classes_taught = $row['classes_taught'];
                        $cl_taught = explode(",",$classes_taught);
            ...
            if ($result) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $status= $row['status'];
                    $ttstatus = "Attended";
                    $inget = "<td><span class='link delete_tt_files'  id='".$row['ids']."'><i class='fa fa-trash'></i> Delete</span> || <span class='link view_timetables' id='".$row['ids']."'><i class='fa fa-eye'></i> View</span></td>";
                    if ($status == 0) {
    ...
    function getSubjectDetails($conn2,$subject_id){
        $select = "SELECT `subject_name`,`timetable_id` FROM `table_subject` WHERE `subject_id` = ?";
        $stmt = $conn2->prepare($select);
        $stmt->bind_param("s",$subject_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result) {
            if ($row = $result->fetch_assoc()) {
                array_push($returndata,$row['subject_name'],$row['timetable_id']);
            }
        }
        return $returndata;
    ...
    function getSubjectName($subjectid,$conn2){
        $select = "SELECT `subject_name` FROM `table_subject` WHERE `subject_id` = ?";
        $stmt = $conn2->prepare($select);
        $stmt->bind_param("s",$subjectid);
        $stmt->execute();
        $res = $stmt->get_result();
