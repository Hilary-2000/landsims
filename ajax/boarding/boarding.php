<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET'){
        ...
        if (isset($_GET['get_dorm_captain'])) {
            ...
            $result = $stmt->get_result();
            if ($result) {
                $data_to_display = "<h6 style='font-size:17px;font-weight:500;text-align:center;margin: 5px 0;'><u>Dormitory List</u></h6><div class='table_holders'><table>
                <tr>
                    <th>No. </th>
                    <th>House Name</th>
                    <th>House Matron</th>
            ...
            if ($result) {
                $data_to_display = "<h6 style='margin-top:10px;text-align:center;font-size:17px;font-weight:500;'>Students to enroll</h6><div class='table_holders'><table>
                                        <tr>
                                            <th>#</th>
                                            <th>Adm no</th>
                                            <th>Student Name</th>
    ...
    function getStudentName($student_id,$conn2){
        ...
        if ($result) {
            if ($row = $result->fetch_assoc()) {
                return explode(",",$row['first_name']." ".$row['second_name'].",".$row['gender']);
            }
        }
